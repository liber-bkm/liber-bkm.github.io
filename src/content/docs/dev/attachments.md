...
Reindex: attachment refs are kept when files are missing (pending, same treatment as md/archive refs); dropped bookmarks' surviving attachments are quarantined to `unindexed/attachments/<rel>`; orphan files under `attachments/` relink by `%04d-` prefix to the bookmark with that id (anything else moves to `unindexed/attachments/`); id compaction (`--compact` only) renames them through the same two-phase stage/commit as the other file kinds, via the `attIdx` field on `pendingMove` (`-1` means a regular html/markdown/archive move).

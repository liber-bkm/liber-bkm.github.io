...
`liber completion bash|zsh|fish` (`completion.go`) prints a static script per shell, embedded as string constants. The scripts never embed a copy of the collection: at completion-time they call back into `liber --tags`, `liber --folders`, and `liber -l` to offer live tag names, folder names, and bookmark ids, so they can't go stale, and they always match the active profile. That's also why `--tags` prints bare names with counts in a parseable layout (dynamic completion is one of its consumers).
